...
{
    public class KingGreatSword:ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.Size = new Vector2(10, 90);
            Item.useTurn = true;

        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            player.itemRotation = -2.05f * player.direction;
    ...
    public class wave : ModProjectile
    {
        ...
        int i = 0;
        float speed = 30;
        Effect DefaultEffect = ModContent.Request<Effect>("LegendMod/Effects/Content/Trail").Value;
        Texture2D MainColor = ModContent.Request<Texture2D>("LegendMod/Images/heatmap").Value;
        Texture2D MainShape = ModContent.Request<Texture2D>("LegendMod/Images/Extra_197").Value;
        Texture2D MaskColor = ModContent.Request<Texture2D>("LegendMod/Images/Extra_189").Value;
        float to;
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.Center = player.RotatedRelativePoint(player.MountedCenter) + Projectile.rotation.ToRotationVector2() * 100f;
            Projectile.ownerHitCheck = true;
            Projectile.timeLeft = (int)(60 / player.GetAttackSpeed(DamageClass.Melee));
            Projectile.rotation = -2.05f * player.direction;
        ...
        public override void AI()
        {
            ...
            if (player.altFunctionUse == 2)
                Projectile.Kill();
            Projectile.Center = player.Center + Projectile.rotation.ToRotationVector2() * 100f;
            
            if(player.direction > 0)
            {
                Projectile.rotation += 0.1f;
                player.itemRotation += 0.1f;
            }
            else
            {
                Projectile.rotation -= 0.1f;
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            for (int i = 1; i < Projectile.oldPos.Length; ++i)
            {
                ...
                var color = new Color(223,173,16);
