...
namespace LegendMod.Common.AuxiliaryMeans
{
    /// <summary>
    /// 提供一些基本的实体AI编写工具
    /// </summary>
    public class AiBehavior
    {
        ...
        /// 检测方块碰撞
        /// </summary>
        public static bool CollTile(NPC entity, int xScope = 1, int yScope = 0)
        {
            Vector2 thisPos = entity.Bottom + new Vector2(0, -16);
            Vector2 entTilePos = WEPosToTilePos(thisPos);
            ...
            for (int y = 0; y <= entTileHeightInt + yScope; y++)
            {
                for (int x = 0; x < xScope; x++)
                {
                    Vector2 ceingTilePos = entTilePos + new Vector2(x * entity.direction, -y) * 16;
        ...
        public static void AccelerationBehavior(Entity entity, Vector2 TargetCenter, float acceleration)
        {
            ...
            Vector2 direction = TargetCenter - entity.Center;
            float distance = direction.Length();

            direction.Normalize();

            Vector2 targetVelocity = direction * distance * acceleration;

            entity.velocity = Vector2.Lerp(entity.velocity, targetVelocity, acceleration);
        }

        public static void NPCToRot(NPC entity, float ToRot, float rotSpeed)
        {
            //entity.rotation = MathHelper.SmoothStep(entity.rotation, ToRot, rotSpeed);
